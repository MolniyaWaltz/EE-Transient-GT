...
% Script to run trial motors and 40Kg of batterys: Expected iridis run time = 6.5 hours
...
generation
    ...
    parfor trial = [1:1:GA.Members]
        [fuel_mass,Transiant_Time] = Run_Trial_APU(Architecture,GA.Trials(trial,generation).Value, Fan_Map, HPC_Map,Combustor,Bypass);
        [Ts_up,POS_up,Tp_up,Ts_down,POS_down,Tp_down] = Run_Step_APU...
    ...
    if generation ~= GA.Generations
        ...
        name = strcat("Winner APU - ",string(cost_ind));
        save(name,'C');
    end
end

save('APU-functions','cost');
save('APU-results','Gen_Results')
save('APU-whole','GA')