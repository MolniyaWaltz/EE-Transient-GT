...
% Script to run trial motors and x Kg of batterys: Expected iridis run time = 6.5 hours
% (For 10 generations with 1000 members, and 32 workers -MW)
...
for generation = [1:1:GA.Generations]
    ...
    if generation ~= GA.Generations
        ...
        if Afterburner.IsActive == 1
            name = strcat("Winner ",string(Storage_Capacity/Storage_Power_Density),"KgAB - ",string(cost_ind));
            save(name,'C');
        else
            name = strcat("Winner ",string(Storage_Capacity/Storage_Power_Density),"Kg - ",string(cost_ind));
            save(name,'C');
        end
    end
end
...
if Afterburner.IsActive == 1
    save(strcat(string(Storage_Capacity/Storage_Power_Density),'KgAB-functions'),'cost');
    save(strcat(string(Storage_Capacity/Storage_Power_Density),'KgAB-results'),'Gen_Results')
    save(strcat(string(Storage_Capacity/Storage_Power_Density),'KgAB-whole'),'GA')
else
    save(strcat(string(Storage_Capacity/Storage_Power_Density),'Kg-functions'),'cost');
    save(strcat(string(Storage_Capacity/Storage_Power_Density),'Kg-results'),'Gen_Results')
    save(strcat(string(Storage_Capacity/Storage_Power_Density),'Kg-whole'),'GA')
end
