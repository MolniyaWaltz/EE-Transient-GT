...
for t = [WS.delta_T:WS.delta_T:WS.Sim_time]
    ...
    if steady_state == 1
        ...
        if (WS.Sim_point + 10) < WS.total_sim_points
            if abs(NH_demand - Senario.Point_Senario((WS.Sim_point + 10),3))>100
                T_Levels(WS.Sim_point) = Fg;
            else
...
for i = [1:1:WS.total_sim_points-1]
    if Senario.Point_Senario(i,3) ~= Senario.Point_Senario(i+1,3)
        T_Levels(i) = NaN;
    end
end
...
% figure(1);
% hold on
% plot([0:WS.delta_T:WS.Sim_time],WS.Tracker(:,1));
% plot([0:WS.delta_T:WS.Sim_time],WS.Tracker(:,2));
% %plot([0:WS.delta_T:WS.Sim_time],WS.Tracker(:,7));
% hold off
% figure(2);
% hold on
% plot([0:WS.delta_T:WS.Sim_time],WS.Tracker(:,8));
% hold off
% figure(3);
% hold on
% plot([0:WS.delta_T:WS.Sim_time],T_Levels);
% hold off
% 
% sum(WS.Tracker(:,9)) * WS.delta_T;
